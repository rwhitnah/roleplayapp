...
export function Combobox<T>({
  ...
  autoFocus?: boolean
  'aria-label'?: string
  children: (value: NonNullable<T>) => any
} & Omit<Headless.ComboboxProps<T, false>, 'as' | 'multiple' | 'children'> & { anchor?: 'top' | 'bottom' }) {

  return (
    <Headless.Combobox {...props} multiple={false} virtual={{ options: options }} onClose={() => {}}>
      <span
        data-slot="control"
        className={clsx([
          className,
      ...
      >
        {({ option }: {option: any}) => children(option)}
      </Headless.ComboboxOptions>
    </Headless.Combobox>
  )
}
...
export function ComboboxOption<T>({
  ...
  className,
  ...props
}: { className?: string; children?: any} & Omit<
...
export function ComboboxLabel({ className, ...props }: any) {
  return <span {...props} className={clsx(className, 'ml-2.5 truncate first:ml-0 sm:ml-2 sm:first:ml-0')} />
}

export function ComboboxDescription({ className, children, ...props }: any) {
  return (
    <span
      {...props}
      className={clsx(
